...
def open_register():
    ...
    Label(reg, text="Username:", bg="lightblue").pack()
    username_entry = Entry(reg)
    username_entry.pack()

    Label(reg, text="Password:", bg="ligihtblue").pack()
    password_entry = Entry(reg, show="*")
    password_entry.pack()

    show_var = IntVar()
    Checkbutton(reg, text="See Password", variable=show_var, command=toggle_password, bg="lightblue").pack()
...
def open_login():
    ...
    message_label = Label(log, text="", fg="white", bg="lightpink")
    message_label.pack()

    Label(log, text="Username:", bg="lightblue").pack()
    username_entry = Entry(log)
    username_entry.pack()

    Label(log, text="Password:", bg="lightblue").pack()
    password_entry = Entry(log, show="*")
    password_entry.pack()

    show_var = IntVar()
    Checkbutton(log, text="See Password", variable=show_var, command=toggle_password, bg="lighblue").pack()
